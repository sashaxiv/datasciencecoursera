...
pollutmean_Kike("~/datasciencecoursera/Curso II - R Programing/Ejercicios/specdata","nitrate",23)


pollutantmean_MegaPro <- function(directory, pollutant, id = 1:332) {
...
pollutantmean_MegaPro("~/datasciencecoursera/Curso II - R Programing/Ejercicios/specdata","nitrate",23)


#####################################################################################
#####################################################################################

##funcionamiento de los .SD

#####################################################################################
#####################################################################################
install.packages("data.table")
library(data.table) 
##.SD stands for "Subset of Data.table". The dot before SD has no significance but doesn't let it 
##clash with a user-defined column name.
...
#Apart from cyl, there are other categorical columns in the dataset such as vs, am, gear and carb. 
#It doesn't really make sense to take the mean of these columns. So let's exclude these columns. This is where .SDcols comes into the picture.

#SDcols
#.SDcols specifies the columns of the data.table that are included in .SD.

#Mean of all columns (continuous columns) in the dataset by number of gears gear, and number of cylinders, 
#cyl, arranged by gear and cyl:
