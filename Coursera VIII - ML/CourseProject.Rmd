...
* Variables with too many NA's
* Variables highly correlated

```{r}
# remove variables with Nearly Zero Variance
zeroSigma_vars <- nearZeroVar(training)
training <- training[, -zeroSigma_vars]
validation  <- validation[, -zeroSigma_vars]
dim(training)

#claeaning variables with too many NAs (>0,95)
Na_vars    <- sapply(training, function(x) mean(is.na(x))) > 0.95
training <- training[, Na_vars==FALSE]
validation  <- validation[, Na_vars==FALSE]
dim(training)
...
validation  <- validation[, -correlated]
dim(training)
```

## Cross Validation.
Let's rebember the key factor in Cross Validation:
* Use the trainingSet
...
```{r}
# create pertiotions with the training dataset 
...
ml_model_rf   <- train(classe ~ ., data=trainSet, trControl=fitControl,method='rf',ntree=100)
